...
function handleToken(
  ...
  if (type === `,` || type === `/`) {
    ...
    const after =
      firstSpace !== -1 ? value.substring(firstSpace + 1) : value.substring(1);
    ast.push(
      space({
    ...
    if (s.peek().type === `(`) {
      ...
      const before =
        args.length && args[0].type === `space`
          ? stringifyCSSValue(args.shift()!)
          : ``;
      const after =
...
function parseNumber(
  ...
  if (numberUnit) {
    const nodeType =
      knownUnits[numberUnit.toLowerCase() as keyof typeof knownUnits] ||
      unknownUnit;
    ast.push(
      nodeType({
        value: numberValue,
