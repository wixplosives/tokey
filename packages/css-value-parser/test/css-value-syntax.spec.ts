...
describe("value-syntax-parser", () => {
  describe("components", () => {
    ...
    it("should parse simple data-type component with range definition", () => {
      expect(parseValueSyntax(`<name [1,1000]>`)).to.eql(
        dataType("name", [1, 1000])
      );
      expect(parseValueSyntax(`<name [ 1 , 1000 ] >`)).to.eql(
        dataType("name", [1, 1000])
      );
  ...
  describe("multipliers", () => {
    ...
    it("literal with multipliers", () => {
      expect(parseValueSyntax(`'name'!`)).to.eql(
        literal("name", true, { range: [1, 1] })
      );
      expect(parseValueSyntax(`'name'?`)).to.eql(
        literal("name", true, { range: [0, 1] })
      );
      expect(parseValueSyntax(`'name'+`)).to.eql(
    ...
    it("keyword with multipliers", () => {
      expect(parseValueSyntax(`name!`)).to.eql(
        keyword("name", { range: [1, 1] })
      );
      expect(parseValueSyntax(`name?`)).to.eql(
        keyword("name", { range: [0, 1] })
      );
      expect(parseValueSyntax(`name+`)).to.eql(
        keyword("name", { range: [1, Infinity] })
      );
      expect(parseValueSyntax(`name*`)).to.eql(
        keyword("name", { range: [0, Infinity] })
      );
      expect(parseValueSyntax(`name{2}`)).to.eql(
        keyword("name", { range: [2, 2] })
      );
      expect(parseValueSyntax(`name{2, 4}`)).to.eql(
        keyword("name", { range: [2, 4] })
      );
    });
    it("group with multipliers", () => {
      expect(parseValueSyntax(`[a]!`)).to.eql(
        group([keyword("a")], { range: [1, 1] })
      );
      expect(parseValueSyntax(`[a]?`)).to.eql(
        group([keyword("a")], { range: [0, 1] })
      );
      expect(parseValueSyntax(`[a]+`)).to.eql(
      ...
      expect(parseValueSyntax(`[a]{2}`)).to.eql(
        group([keyword("a")], { range: [2, 2] })
      );
      expect(parseValueSyntax(`[a]{2, 4}`)).to.eql(
        group([keyword("a")], { range: [2, 4] })
      );
    });
  });
      ...
      expect(parseValueSyntax(`a b | c || d && e f`)).to.eql(
        bar([
          ...
          doubleBar([
            ...
            doubleAmpersand([
              keyword("d"),
              juxtaposing([keyword("e"), keyword("f")]),
            ]),
          ]),
        ])
      );
